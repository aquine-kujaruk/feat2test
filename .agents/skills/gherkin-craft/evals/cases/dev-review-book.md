...
Earlier context: Build a skill that guides an agent reviewing code changes using the book. The outcome is evidence-backed review findings while preserving project files. Tests exercise the agent with that skill.

# Request

Use gherkin-craft to turn the following book notes into .feature.md specifications in output/. Deliver the specifications and indispensable open decisions only.
