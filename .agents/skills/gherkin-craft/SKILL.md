---
name: gherkin-craft
description: "Guide agents in specifying executable assets through domain rules and complete use cases. Use to write, review, or reformulate native .feature and Markdown .feature.md Gherkin, or define intended software, skill, prompt, and plugin behavior from requirements, procedures, books, articles, or transcripts. Ordinary summaries, prose editing, and implementation alone do not need this workflow."
---
...
**Contextual intent → intended executable asset → relevant domain → complete
use cases → verification targets.**
...
answers or changed or preserved state. The intended asset can be planned;
specification does not require its implementation or tests to exist.

## Shared contract

- Reuse the objective already available in conversation or project context.
  Sources supply knowledge for that objective; their chapters or medium do not
  select the system to build. If incompatible objectives remain, expose that
  decision for the calling agent to resolve. Do not silently select an asset or
  present source-only Features as a complete contract. There is no separate
  skill-owned interview or compulsory input form.
...
- Keep this working model internal unless the user asks to inspect it. Deliver
  ...
  architecture document, catalog application, tests, or implementation by default.
- Specify domain consequences. Implementation mechanisms belong only when
  they are themselves the subject being specified. Source claims retain their
...
[verification targets](references/verification-targets.md) to label each
completed Feature by the asset boundaries its tests will exercise.
...
Preserve subjects, units, scope, timing, quantifiers, negation, relative selectors,
and causal meaning. Use explicit absence rather than blank cells or `null` /
`undefined` stand-ins. A literal that is itself the source's subject remains
literal data. Supply source inputs when their derivation is being examined.
...
and evidence distinctions, coverage claims, and open decisions. A summary of the
